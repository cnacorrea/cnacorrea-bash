...
# sysadmin-friendly prompt, and can be used enforce system's default umask,
...
# [*network_proxy*]
#   If non-empty, sets network proxy to be used.
#   The expected format is either fqdn:port or ipaddress:port.
#   It is assumed that the proxy server is able to provide HTTP/S and
#   FTP proxying, so environment vars for the 3 protocols are set
#   (http_proxy, https_proxy and ftp_proxy). Default: undef
# [*timeout*]
#   Time (seconds) idle sessions will wait before auto-logoff. Default: 600
# [*header*]
#   String containing the header for the generated profile script,
#   maybe warning fellow sysadmins that the file is being managed by Puppet.
#   simple comment is set as default.
# [*color*]
